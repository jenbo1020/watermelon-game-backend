...
async function default_1(call) {
    ...
    if (userReceive) {
        if (taskInfo.refreshType != TaskRefreshType_1.ETaskRefreshType.always && userReceive.createTime >= startTime) {
            return call.error('任务已领取');
        }
    }
    await Global_1.Global.collection('TaskUserReceive').insertOne({
